...
export default function LoginPage() {
  ...
  const [keyauthEnabled, setKeyauthEnabled] = useState(true);

  useEffect(() => {
    fetch('/downloads/settings.json')
      .then(res => res.json())
      .then(data => setKeyauthEnabled(data.keyauth_enabled ?? true))
      .catch(() => setKeyauthEnabled(true));
  }, []);

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!keyauthEnabled) return;
    setLoading(true);
    setError('');
      ...
      <div className="w-full max-w-md space-y-8 glass-card p-8 animate-in fade-in zoom-in duration-500">
        ...
        <div className="space-y-4 mt-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-black font-bold py-3 px-4 rounded-lg transition-all active:scale-95 border border-zinc-200"
          >
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-black px-2 text-zinc-500">
                {keyauthEnabled ? 'O usa KeyAuth' : 'KeyAuth desactivado'}
              </span>
            </div>
          </div>
        </div>
        ...
        {keyauthEnabled ? (
          ...
          <div className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-lg text-center text-zinc-500 text-sm">
            El inicio de sesión mediante KeyAuth ha sido desactivado por el administrador.
          </div>
        )}
